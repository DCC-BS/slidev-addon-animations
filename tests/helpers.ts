...
import type { AnimationTarget, EasingFunction, AnimatableObject } from '../types/animation.js'
...
export const expectAnimationToComplete = async (
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    setTimeout(() => reject(new Error(`Animation did not complete within ${timeout}ms`)), timeout)
  })
...
export const expectValuesNear = (
  ...
  for (const [key, expectedValue] of Object.entries(expected)) {
    ...
    if (typeof actualValue !== 'number') {
      throw new Error(`Expected ${key} to be a number, got ${typeof actualValue}`)
    }
...
export class TimeController {
  ...
  async runUntil(condition: () => boolean, maxTime: number = 5000): Promise<void> {
 ...
 */
export const createMockKonvaNode = (props: Partial<AnimatableObject> = {}): AnimatableObject => ({
  x: 0,
  y: 0,
  scaleX: 1,
  scaleY: 1,
...
export const createMockSvgElement = (props: Partial<AnimatableObject> = {}): AnimatableObject => ({
  x: 0,
  y: 0,
  width: 100,
  height: 100,
...
export const expectPropertiesUpdated = (
  ...
  for (const [key, expectedValue] of Object.entries(expectedUpdates)) {
    ...
    if (typeof expectedValue === 'number' && typeof actualValue === 'number') {
      expectValuesNear({ [key]: actualValue }, { [key]: expectedValue }, tolerance)
    } else {
