...
describe('Animation Integration', () => {
  ...
  it('should handle animation loop lifecycle', () => {
    const animations = prepareAnimations([animationTarget], 0, 100, undefined)
    const onComplete = vi.fn()

    // Create animation loop
    const { frameId, cancel } = createAnimationLoop(animations, onComplete, 1)
  ...
  it('should handle multiple animation steps correctly', () => {
    // Test step 0
    let animations = prepareAnimations([animationTarget], 0, 1000, undefined)
  ...
  it('should handle out-of-bounds step indices gracefully', () => {
    ...
    let animations = prepareAnimations([animationTarget], 10, 1000, undefined)
  ...
  it('should use correct duration and delay from steps', () => {
    const customTarget = createAnimationTarget(
      target,
      { x: 0 },
      [
        createAnimationStep(
    ...
    const animations = prepareAnimations([customTarget], 0, 1000, undefined)
  ...
  it('should fall back to default values when step values are undefined', () => {
    const targetWithDefaults = createAnimationTarget(
      target,
      { x: 0 },
      [
    ...
    const animations = prepareAnimations([targetWithDefaults], 0, 2000, undefined)
