...
describe("Step with Delays Bug Investigation", () => {
    ...
    beforeEach(() => {
        fadingElements = [{ opacity: 0 }, { opacity: 0 }, { opacity: 0 }];
    });
    ...
    it("should animate elements with staggered timing", () => {
        ...
        const updates500 = processAnimationFrame(
            animations,
            startTime + 500,
            startTime,
        );
        ...
        const updates1500 = processAnimationFrame(
            animations,
            startTime + 1500,
            startTime,
        );
        ...
        const firstUpdate1500 = updates1500.find(
            (u) => u.target === fadingElements[0],
        );
        const secondUpdate1500 = updates1500.find(
            (u) => u.target === fadingElements[1],
        );
        ...
        const updates2000 = processAnimationFrame(
            animations,
            startTime + 2000,
            startTime,
        );
        ...
        const secondUpdate2000 = updates2000.find(
            (u) => u.target === fadingElements[1],
        );
        const thirdUpdate2000 = updates2000.find(
            (u) => u.target === fadingElements[2],
        );
        ...
        const updates2500 = processAnimationFrame(
            animations,
            startTime + 2500,
            startTime,
        );
        ...
        const updateInfo = updates2500.map((update) => ({
            target: fadingElements.indexOf(
                update.target as { opacity: number },
            ),
