...
import type { AnimationTarget, EasingFunction, ProcessedAnimation } from '../types/animation.js'
...
describe('animationEngine', () => {
  ...
  describe('prepareAnimations', () => {
    it('should prepare animations for a valid step', () => {
      const animations = prepareAnimations([animationTarget], 0, 1000, undefined)
    ...
    it('should use step-specific duration and delay', () => {
      const animations = prepareAnimations([animationTarget], 1, 1000, undefined)
    ...
    it('should return empty array for invalid step index', () => {
      const animations = prepareAnimations([animationTarget], 5, 1000, undefined)
    ...
    it('should use default values when step values are undefined', () => {
      ...
      const animations = prepareAnimations([targetWithoutDuration], 0, 2000, undefined)
  ...
  describe('prepareReverseAnimations', () => {
    ...
    it('should prepare reverse animations to step 0', () => {
      const animations = prepareReverseAnimations([animationTarget], 0, 1000, undefined)
    ...
    it('should prepare reverse animations to initial state', () => {
      const animations = prepareReverseAnimations([animationTarget], -1, 1000, undefined)
  ...
  describe('applyEasing', () => {
    ...
    it('should apply easing function correctly', () => {
      const mockEasing: EasingFunction = vi.fn((t, _b, c, _d) => c * t * t) // quadratic
  ...
  describe('processAnimationFrame', () => {
    ...
    beforeEach(() => {
      animations = prepareAnimations([animationTarget], 0, 1000, undefined)
    })
    ...
    it('should process animation frame correctly', () => {
      ...
      const batchUpdates = processAnimationFrame(animations, currentTime, startTime)
  ...
  describe('createAnimationLoop', () => {
    it('should create and start animation loop', () => {
      ...
      const { frameId, cancel } = createAnimationLoop(animations, onComplete)
  ...
  describe('Animation Delays', () => {
    ...
    describe('Multiple Animations with Different Delays', () => {
      ...
      it('should apply animations independently based on their individual delays', () => {
        const animations = prepareAnimations(multiTarget, 0, 1000, undefined)
      ...
      it('should apply both animations after both delays have passed', () => {
        ...
        const firstUpdate = batchUpdates.find(update => update.target === multiTarget[0].target)
        ...
        const secondUpdate = batchUpdates.find(update => update.target === multiTarget[1].target)
      ...
      it('should complete animations independently based on their total time', () => {
        ...
        console.log('Animation 0 - delay:', animations[0].delay, 'duration:', animations[0].duration, 'completed:', animations[0].completed)
        console.log('Animation 1 - delay:', animations[1].delay, 'duration:', animations[1].duration, 'completed:', animations[1].completed)
        console.log('Expected completion times:')
        console.log('Animation 0: startTime(1000) + delay(200) + duration(1000) = 2200ms')
        console.log('Animation 1: startTime(1000) + delay(500) + duration(1000) = 2500ms')
    ...
    describe('Animation Groups (Steps) with Delays', () => {
      ...
      it('should apply step 0 immediately (no delay)', () => {
        const animations = prepareAnimations([groupTarget], 0, 1000, undefined)
      ...
      it('should respect delay when transitioning to step 1', () => {
        const animations = prepareAnimations([groupTarget], 1, 1000, undefined)
      ...
      it('should handle step 2 with its own delay correctly', () => {
        ...
        const animations = prepareAnimations([groupTarget], 2, 1000, undefined)
    ...
    describe('Delay Edge Cases', () => {
      ...
      it('should handle delay longer than animation duration', () => {
        ...
        const animations = prepareAnimations([shortDurationTarget], 0, 1000, undefined)
        ...
        const animations2 = prepareAnimations([shortDurationTarget], 0, 1000, undefined)
        
        processAnimationFrame(animations2, exactCompletionTime, startTime)
      ...
      it('should handle fractional delays', () => {
        ...
        let batchUpdates = processAnimationFrame(animations, currentTime, startTime)
        expect(batchUpdates).toHaveLength(0)
        
        const currentTime2 = 1251 // 251ms elapsed (after delay ends)
        batchUpdates = processAnimationFrame(animations, currentTime2, startTime)
        expect(batchUpdates).toHaveLength(1)
      })
    })
    ...
    describe('Animation Loop with Delays', () => {
      it('should call onComplete only after all delayed animations finish', () => {
        ...
        const animations = prepareAnimations(delayedTargets, 0, 1000, undefined)
        const onComplete = vi.fn()
    ...
    describe('Complex Real-World Delay Scenarios', () => {
      it('should handle staggered animations with different delays and durations', () => {
        ...
        const animations = prepareAnimations(staggeredTargets, 0, 1000, undefined)
        const startTime = 1000
        
        // Test at 150ms: only first animation should be running
        const updates150 = processAnimationFrame(animations, startTime + 150, startTime)
        expect(updates150).toHaveLength(2) // First (running) + Second (just started)
        
        // First animation at 50% progress (150/300)
        const firstUpdate = updates150.find(u => u.target === element1)
        ...
        const secondUpdate = updates150.find(u => u.target === element2)
        ...
        const updates350 = processAnimationFrame(animations, startTime + 350, startTime)
      ...
      it('should handle cascading animations where delay equals previous animation duration', () => {
        ...
        const animations = prepareAnimations(cascadingTargets, 0, 1000, undefined)
        const startTime = 1000
        
        // At 100ms: only first animation should be at 50%
        const updates100 = processAnimationFrame(animations, startTime + 100, startTime)
        ...
        const updates200 = processAnimationFrame(animations, startTime + 200, startTime)
        expect(updates200).toHaveLength(2)
        expect(animations[0].completed).toBe(true)
        
        const secondUpdate = updates200.find(u => u.target === element2)
        expect(secondUpdate?.updates.scale).toBe(1) // Just started (0% progress)
        
        // At 300ms: first done, second at 50%
        const updates300 = processAnimationFrame(animations, startTime + 300, startTime)
