...
const circle1 = ref<CircleConfig>({ x: 100, y: 200, radius: 40, fill: 'red', opacity: 1, scaleX: 1, scaleY: 1 });
const circle2 = ref<CircleConfig>({ x: 300, y: 200, radius: 30, fill: 'blue', opacity: 0, scaleX: 0.5, scaleY: 0.5 });
const rect1 = ref<RectConfig>({ x: 500, y: 150, width: 80, height: 60, fill: 'green', rotation: 0, opacity: 1 });

// Animation generator function demonstrating various animation types
function* animationSequence() {
    // Step 1: Basic movement and scaling
    yield step(
        moveTo(circle1, 200, 200, { duration: 1000, easing: EasingPresets.easeOut }),
    ...
    yield step(
        moveTo(circle1, 350, 100, { duration: 1200, easing: EasingPresets.elasticOut }),
        moveTo(circle2, 150, 300, { duration: 1000, easing: EasingPresets.backOut }),
        rotateTo(rect1, Math.PI / 4, { duration: 800, easing: EasingPresets.easeInOut })
    );

    // Step 3: Complex choreographed movement
    yield step(
        animate(circle1, { x: 400, y: 250, fill: 'orange' }, { duration: 1000 }),
        animate(circle2, { x: 250, y: 150, radius: 50 }, { duration: 1200, easing: EasingPresets.elasticOut }),
        animate(rect1, { x: 300, y: 300, width: 120, height: 40 }, { duration: 800 })
    );

    // Step 4: Scale and fade effects
    yield step(
        scaleTo(circle1, 1.5, { duration: 600, easing: EasingPresets.bounceOut }),
        fadeTo(rect1, 0.3, { duration: 800 }),
        animate(circle2, { fill: 'purple' }, { duration: 400 })
    );

    // Step 5: Reset to original positions
    yield step(
        moveTo(circle1, 100, 200, { duration: 1000, easing: EasingPresets.easeInOut }),
        moveTo(circle2, 300, 200, { duration: 1000, easing: EasingPresets.easeInOut }),
        animate(rect1, {
            x: 500,
            y: 150,
            rotation: 0,
